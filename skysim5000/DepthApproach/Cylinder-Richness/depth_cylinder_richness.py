...
file_path = r"/global/u1/k/kamau/SE-CLMM-LSSTDESC/project-1/Data/halos/"
filepath = '/global/u1/k/kamau/SE-CLMM-LSSTDESC/project-1/Notebooks/skysim5000/Depth/calc-richness/DepthData/'
...
depth = args.depth
chisq_cut = 20  # 20 #1e10 # TODO: no color cut for now

## Read in All the gcr halos
sky_02_1_all = Table.read(file_path + 'skysim_0.2-1-2152757_comv&DA.dat', format='ascii')
# sky_02_1_all = sky_02_1_all[sky_02_1_all['halo_id']==54310048365] # 1 cluster

### Check whether cylinder richness has already been computed to avoid repetition
# lambda_60 = Table.read(filepath + f'RichnessDepth-Data/RichnessDepth_{depth}_035_05.dat', format='ascii')
...
class AlternativeRichness(object):
    ...
    def get_galaxies(self):
      ...
      #### Step 2: cut redshift and magnitude
        ...
        sel = (z_all >= self.zmin-1)&(z_all <= self.zmax+1)  & (mag_z_all > 10) & (mag_z_all < z_vs_redshift(self.zmid))  # & mag_i_all > 10)&(mag_i_all < self.mag_i_cut) & (i_vs_redshift(self.zmid))
