...
output_file_path = file_path + 'Notebooks/skysim5000/Depth/calc-richness/Data-035-05/thirty/' # TODO: Gladys, please set it to your output directory
# input_file_path = file_path + './'

depth = 30
chisq_cut = 9 #1e10 # TODO: no color cut for now

## Read in All the gcr halos
sky_02_1_all = Table.read(file_path + 'Data/halos/skysim_0.2-1-2152757.dat', format='ascii')

# ### Check whether cylinder richness has already been computed to avoid repetition
# lambda_60 = Table.read(file_path + 'Data/halos/RichnessDepth_30_035_05.dat', format='ascii')
...
class AlternativeRichness(object):
    ...
    def get_galaxies(self):
      ...
      #### Step 2: cut redshift and magnitude
        ...
        sel = (z_all >= self.zmin-0.1)&(z_all <= self.zmax+0.1)  & (mag_z_all > 10) & (mag_z_all < z_vs_redshift(self.zmid)) # & mag_i_all > 10)&(mag_i_all < self.mag_i_cut) & (i_vs_redshift(self.zmid))

        # print('done selection')
        self.chi_gal = chi_all[sel] #Mpc
    ...
    def get_halos(self):
        ...
        self.chi_halo = cosmo.comoving_distance(self.redshift_halo).value #Mpc
    ...
    def get_richness(self, i_halo):
        ...
        DA = self.chi_halo[i_halo] / (1+self.redshift_halo[i_halo]) # Mpc
        r = DA * ang_sep # physical Mpc
